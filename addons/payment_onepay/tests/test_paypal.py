...
class OnepayForm(OnepayCommon):
    ...
    def test_10_onepay_form_render(self):
        ...
        self.onepay.write({'onepay_email_account': 'tde+onepay-facilitator@odoo.com', 'fees_active': False})
        self.assertEqual(self.onepay.state, 'test', 'test without test environment')
        ...
        # render the button
        res = self.onepay.render(
            'test_ref0', 0.01, self.currency_euro.id,
            values=self.buyer_values)

        form_values = {
            'cmd': '_xclick',
            'business': 'tde+onepay-facilitator@odoo.com',
            'item_name': '%s: test_ref0' % (self.onepay.company_id.name),
        ...
        self.assertEqual(len(data_set), 1, 'onepay: Found %d "data_set" input instead of 1' % len(data_set))
        self.assertEqual(data_set[0].get('data-action-url'), 'https://www.sandbox.onepay.com/cgi-bin/webscr', 'onepay: wrong form POST url')
    ...
    def test_11_onepay_form_with_fees(self):
        # be sure not to do stupid things
        self.assertEqual(self.onepay.state, 'test', 'test without test environment')
        ...
        for form_input in tree.input:
            if form_input.get('name') in ['handling']:
                handling_found = True
                self.assertEqual(form_input.get('value'), '1.57', 'onepay: wrong computed fees')
        self.assertTrue(handling_found, 'onepay: fees_active did not add handling input in rendered form')

    @mute_logger('odoo.addons.payment_onepay.models.payment', 'ValidationError')
    def test_20_onepay_form_management(self):
        # be sure not to do stupid things
        self.assertEqual(self.onepay.state, 'test', 'test without test environment')

        # typical data posted by onepay after client has successfully paid
        onepay_post_data = {
        ...
        with self.assertRaises(ValidationError):
            self.env['payment.transaction'].form_feedback(onepay_post_data, 'onepay')

        # create tx
        tx = self.env['payment.transaction'].create({
            'amount': 1.95,
            'acquirer_id': self.onepay.id,
        ...
        # validate it
        tx.form_feedback(onepay_post_data, 'onepay')
        # check
        self.assertEqual(tx.state, 'pending', 'onepay: wrong state after receiving a valid pending notification')
        self.assertEqual(tx.state_message, 'multi_currency', 'onepay: wrong state message after receiving a valid pending notification')
        self.assertEqual(tx.acquirer_reference, '08D73520KX778924N', 'onepay: wrong txn_id after receiving a valid pending notification')
        ...
        # update notification from onepay
        onepay_post_data['payment_status'] = 'Completed'
        # validate it
        tx.form_feedback(onepay_post_data, 'onepay')
        # check
        self.assertEqual(tx.state, 'done', 'onepay: wrong state after receiving a valid pending notification')
        self.assertEqual(tx.acquirer_reference, '08D73520KX778924N', 'onepay: wrong txn_id after receiving a valid pending notification')
        self.assertEqual(fields.Datetime.to_string(tx.date), '2013-11-18 11:21:19', 'onepay: wrong validation date')

    def test_21_onepay_compute_fees(self):
        ...
        total_fee = self.onepay.onepay_compute_fees(100, False, False)
