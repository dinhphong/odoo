...
class AcquirerOnepay(models.Model):
    ...
    onepay_email_account = fields.Char('Email', required_if_provider='onepay', groups='base.group_user')
    onepay_seller_account = fields.Char(
    ...
    onepay_use_ipn = fields.Boolean('Use IPN', default=True, help='Onepay Instant Payment Notification', groups='base.group_user')
    onepay_access_code = fields.Char(string='Access Code', default='D67342C2', help='Access Code provided by ONEPAY', groups='base.group_user')
    onepay_secret_hash = fields.Char(string='Secret Hash', default='A3EFDFABA8653DF2342E8DAC29B51AF0', help='Secret Hash provided by ONEPAY', groups='base.group_user')
    onepay_user = fields.Char(string='User',default='op01', help='User provided by ONEPAY', groups='base.group_user')
    onepay_password = fields.Char(string='Password',default='op123456', help='Password provided by ONEPAY', groups='base.group_user')
    # Default onepay fees
    ...
    def _get_onepay_urls(self, environment = ''):
        """ Onepay URLS """
        return {
            'onepay_form_url': 'https://onepay.vn/paygate/vpcpay.op',
            'onepay_rest_url': 'https://onepay.vn/onecomm-pay/Vpcdps.op',
        }

    def onepay_compute_fees(self, amount, currency_id, country_id):
        """ Compute onepay fees.
    ...
    def onepay_form_generate_values(self, values):
        ...
        onepay_tx_values.update({
            ...
            'vpc_Amount'        : str(float(values['amount']) * 100).replace('.0',''),
        ...
        sorted_values = OrderedDict(sorted(onepay_tx_values.items(), key=lambda x: x[0]))
        ...
        secret = bytes.fromhex(self.onepay_secret_hash)
    ...
    def onepay_get_query_dr_url(self):
        self.ensure_one()
        environment = 'prod' if self.state == 'enabled' else 'test'
        return self._get_onepay_urls(environment)['onepay_rest_url']
    

class TxOnepay(models.Model):
    _inherit = 'payment.transaction'

    onepay_txn_type = fields.Char('Transaction type')
    ...
    def _onepay_form_get_tx_from_data(self, data):
        ...
        if not reference or not txn_id:
            error_msg = _('Onepay: received data with missing reference (%s) or txn_id (%s)') % (reference, txn_id)
        ...
        if not txs or len(txs) > 1:
            error_msg = 'Onepay: received data for reference %s' % (reference)
            if not txs:
    ...
    def _onepay_form_get_invalid_parameters(self, data):
        ...
        # check seller
        if data.get('receiver_id') and self.acquirer_id.onepay_seller_account and data['receiver_id'] != self.acquirer_id.onepay_seller_account:
            invalid_parameters.append(('receiver_id', data.get('receiver_id'), self.acquirer_id.onepay_seller_account))
        if not data.get('receiver_id') or not self.acquirer_id.onepay_seller_account:
            ...
            if data.get('receiver_email') and data.get('receiver_email') != self.acquirer_id.onepay_email_account:
                invalid_parameters.append(('receiver_email', data.get('receiver_email'), self.acquirer_id.onepay_email_account))
            if data.get('business') and data.get('business') != self.acquirer_id.onepay_email_account:
                invalid_parameters.append(('business', data.get('business'), self.acquirer_id.onepay_email_account))
    ...
    def _onepay_form_validate(self, data):
        ...
        res = {
            ...
            'onepay_txn_type': data.get('vpc_Command'),
        }
        if not self.acquirer_id.onepay_access_code and not self.acquirer_id.onepay_seller_account and status == 0:
            template = self.env.ref('payment_onepay.mail_template_onepay_invite_user_to_configure', False)
            if template:
                ...
                mail_values = {
                    ...
                    'email_to': self.acquirer_id.onepay_email_account,
