...
class OnepayController(http.Controller):
    _notify_url = '/payment/onepay/ipn/'
    _return_url = '/payment/onepay/dpn/'
    ...
    def onepay_validate_data(self, **post):
        """ Onepay IPN: three steps validation to ensure data correctness
         ...
         - step 3: onepay send either VERIFIED or INVALID (single word) for IPN
        ...
        if not tx:
            ...
            # odoo, acknowledge it otherwise onepay will keep trying
            _logger.warning('received notification for unknown payment reference')
            return False

        onepay_url = tx.acquirer_id.onepay_get_query_dr_url()

        params = dict({
            'vpc_AccessCode': tx.acquirer_id.onepay_access_code,
            'vpc_Command': 'queryDR',
            'vpc_Version': '1',
            'vpc_MerchTxnRef': post['vpc_MerchTxnRef'],
            'vpc_Merchant': tx.acquirer_id.onepay_seller_account,
            'vpc_Password': tx.acquirer_id.onepay_password,
            'vpc_SecureHash': '',
            'vpc_User': tx.acquirer_id.onepay_user
        })

        sorted_values = OrderedDict(sorted(params.items(), key=lambda x: x[0]))  
        secret = bytes.fromhex(tx.acquirer_id.onepay_secret_hash)
        ...
        urequest = requests.get(onepay_url + '?' + url)
        ...
        if resp == 'SUCCESS':
            _logger.info('Onepay: validated data')
            res = request.env['payment.transaction'].sudo().form_feedback(post, 'onepay')
            if not res and tx:
                tx._set_transaction_error('Validation error occured. Please contact your administrator.')
        elif resp == 'FAIL':
            _logger.warning('Onepay: answered INVALID/FAIL on data verification')
            if tx:
                tx._set_transaction_error('Transaction failed, please try again or contact administrator.')
        else:
            _logger.warning('Onepay: unrecognized onepay answer, received %s instead of VERIFIED/SUCCESS or INVALID/FAIL (validation: %s)' % (resp, 'IPN/DPN'))
            if tx:
                tx._set_transaction_error('Unrecognized error from Onepay. Please contact your administrator.')
        return res

    @http.route('/payment/onepay/ipn/', type='http', auth='public', methods=['POST'], csrf=False)
    def onepay_ipn(self, **post):
        """ Onepay IPN. """
        _logger.info('Beginning Onepay IPN form_feedback with post data %s', pprint.pformat(post))  # debug
        try:
            self.onepay_validate_data(**post)
        except ValidationError:
            _logger.exception('Unable to validate the Onepay payment')
        return ''

    @http.route('/payment/onepay/dpn', type='http', auth="public", methods=['POST', 'GET'], csrf=False)
    def onepay_dpn(self, **post):
        """ Onepay DPN """
        _logger.info('Beginning Onepay DPN form_feedback with post data %s', pprint.pformat(post))  # debug
        try:
            res = self.onepay_validate_data(**post)
        except ValidationError:
            _logger.exception('Unable to validate the Onepay payment')
        return werkzeug.utils.redirect('/payment/process')

    @http.route('/payment/onepay/cancel', type='http', auth="public", csrf=False)
    def onepay_cancel(self, **post):
